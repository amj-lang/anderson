...
[![version](https://img.shields.io/badge/version-0.29.1-blue)](https://github.com/amj-lang/anderson/releases)
...
- **Jack in**: `⏎` switches tmux to that session's pane; without tmux, on macOS it focuses the iTerm2 / Terminal.app tab that owns the session. `w` jumps to the oldest one waiting. `r` kills (asks first). `b` dismisses a dead row. `/` filters. `?` manual.
