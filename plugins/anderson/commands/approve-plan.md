...
1. Set stage=implement, then print this IMPLEMENT banner (pick ONE quote at random — never default to the first, and don't reuse one you showed earlier this session) as the LAST line before invoking the implementer:
   ```
...
2. Print this DIFF-REVIEW banner (pick ONE quote at random — never default to the first, and don't reuse one you showed earlier this session) as the LAST line before invoking the reviewer:
   ```
