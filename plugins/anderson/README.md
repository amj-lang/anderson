...
[![version](https://img.shields.io/badge/version-0.37.2-blue)](https://github.com/amj-lang/anderson)
...
- **0.37.2** — **Fleet: no ping when you're already there.** A ring skips the desktop banner and the sound when the
...
- **0.37.2** — **State first.** `/anderson:start` now seeds `feature-research/<task>/state.md` in a shell preamble,
...
- **0.37.2** — **Fleet: `h` shows the hidden rows** (flagged `◌`, dim, counted in the header); `b` on one brings it
...
- **0.37.2** — **Fleet: kill hides, hide never kills.** `r` (kill, asks first) now hides the row as soon as the signal
...
- **0.37.2** — **Fleet: the footer moves to the floor.** Keys and usage now sit on the last lines of the terminal, each on
...
`subagents/` folder.
- **0.37.2** — `fleet --play all` auditions every ring sound from the shell (`--play NAME` for one); `--rings` says so.
- **0.37.2** — **Fleet: pick your ring.** Seven bundled sounds in `assets/sounds/` (phone, the Matrix call, stays the default;
...
- **0.37.2** — **Fleet: one process, one row.** Headless `claude -p` children spawned inside a session (reviewers, hooks) no longer show up as a second session of the same repo, and after `/clear` or `/resume` only the process's current session id is listed.
